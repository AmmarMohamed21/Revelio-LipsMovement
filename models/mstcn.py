...
def load_mstcn_model(model_path, configfile='configs/mstcn.json', device="cuda:0"):
    tcn_options = {}
    #parse config file
    with open(configfile, 'r') as f:
        json_options = json.load(f)
        tcn_options = json_options
    model = MultiscaleMultibranchTCN(input_size=512,
            num_channels=[tcn_options["hidden_dim"] * len(tcn_options["kernel_size"]) * tcn_options["width_mult"]]
